...
| 顺序 | 内容 | 预计时间 | 状态 |
| --- | --- | ---: | --- |
| 20 | [[20_路由表从哪里来_转发平面与控制平面|路由表从哪里来：转发平面、控制平面与路由收敛]] | 1.75～2.5 小时 | 基本掌握；三道收尾题待答 |
...
后续不再连续安排填补预设函数的 Lab，而是保留两个从空项目开始的综合项目：

1. **TinyRouter（8～12 小时）**：自己参与确定类、状态、接口和调用关系，再逐步实现网络接口、ARP、路由表与多接口路由器。
2. **TinyNetworkApp（8～12 小时）**：使用真实 TCP/UDP Socket，实现序列化、消息边界、超时和不同可靠性需求。

排队与拥塞控制改为 1～2 小时的短模拟。每个项目拆成单次 1～2.5 小时的阶段，不要求单独实验报告。
