...
- 上一课：[[27_DNS_域名怎样找到服务器|第 27 课：域名怎样找到服务器？——DNS 查询、委派与缓存]]（基本掌握；两道收尾题通过）
...
- [[27_DNS_域名怎样找到服务器|第 27 课：域名怎样找到服务器？——DNS 查询、委派与缓存]] 基本掌握；解析器角色与具体诊断收尾已通过。
...
当前仍从第 28 课开始顺序读。后续正文已齐，不要求一次看完；不会把“发布”当成“掌握”。

1. [[28_NAT_端口映射与公网连入|第 28 课：NAT、端口映射与公网连入]]（原课保留）
2. [[29_HTTP_通用请求与响应|第 29 课：HTTP 通用请求与响应]] · [交互](29_HTTP_响应与缓存_交互.html)
3. [[30_WebSocket_长连接与断线恢复|第 30 课：WebSocket、心跳与断线恢复]] · [交互](30_连接恢复_交互.html)
4. [[31_TLS_加密身份与权限|第 31 课：TLS、身份与权限]] · [交互](31_安全边界_交互.html)
5. [[32_完整网络旅程与故障定位|第 32 课：完整网络旅程与故障定位]] · [交互](32_故障定位_交互.html)
6. [[第一阶段_结课检查与针对性补充|结课检查：五组情境，只补缺口]]

从第 29 课到结课约 12～17 小时；若连尚未完成的第 28 课一起算，约 14～20 小时。时间包含推演、回答与反馈，不为达到估时而增加重复阅读。
