...
你的电脑并不是临时猜“该向谁问 DNS”。它从网络配置中知道一个 DNS 解析器的地址：可能是连入网络时获得的，也可能是你手动填写的。电脑先把问题交给它。假设相关缓存都没有答案，这个解析器也不知道游戏服务器的 IP，于是逐级去找负责发布记录的人：
...
这里“问根、问 `.com`”是解析器分别发出的 DNS 查询，并不是 IP 数据报经过路由器时逐跳询问。根服务器和 `.com` 服务器主要指出下一位该问谁；保存这条例子中最终 A 记录的是 `example.com` 的权威服务器。
